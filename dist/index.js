...
__export(index_exports, {
  ...
  DEFAULT_LANGUAGE_DETECTOR_CONFIG: () => DEFAULT_LANGUAGE_DETECTOR_CONFIG,
  ...
  KondLanguageDetector: () => KondLanguageDetector,
  ...
  createKondLanguageDetector: () => createKondLanguageDetector,
  createMediaRecorderAdapter: () => createMediaRecorderAdapter,
  createMockLanguageDetector: () => createMockLanguageDetector,
  ...
  languageToLocale: () => languageToLocale,
...
var AudioWorkletRecordingAdapter = class {
  ...
  handleWorkletMessage(data) {
    switch (data.type) {
      case "audioChunk":
        if (data.samples && (this.state === "recording" || this.state === "processing")) {
  ...
  addAssistantAudio(audioData) {
    if (this.state !== "recording" || !this.mixerWorklet) {
      if (this.state !== "idle") {
        console.warn(
          `[${this.name}] addAssistantAudio dropped: state=${this.state}, worklet=${!!this.mixerWorklet}, samples=${audioData.length}`
        );
      }
      return;
    }
...
// src/adapters/language-detector/mock.ts
var MockLanguageDetector = class {
  constructor(options = {}) {
    this.name = "mock";
    this.currentLocale = null;
    this.options = {
   ...
   */
  setLanguage(language, confidence = 0.95) {
    this.options.language = language;
    this.options.confidence = confidence;
  }
};
function createMockLanguageDetector(options) {
