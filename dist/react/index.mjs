...
// src/ports/language-detector.ts
...
var DEFAULT_LANGUAGE_DETECTOR_CONFIG = {
  baseUrl: DEFAULT_BASE_URL,
  timeoutMs: 2e3,
  confidenceThreshold: 0.8,
  debug: false
...
// src/adapters/language-detector/kond.ts
var KondLanguageDetector = class {
  constructor(options) {
    this.name = "kond";
...
function createKondLanguageDetector(options) {
...
function createTurnManager(config2) {
  ...
  return {
    handleTranscript(text, isFinal, speechFinal, conf) {
      ...
      if (isFinal) {
        if (cfg.turnDetector && !pendingPrediction) {
          const predGen = ++predictionGeneration;
          pendingPrediction = getMlPrediction().finally(() => {
            if (predGen === predictionGeneration) {
    ...
    getLocale() {
      return cfg.locale || "fr";
    },
    setLocale(locale) {
      if (cfg.locale !== locale) {
        log("Locale changed:", cfg.locale, "\u2192", locale);
        cfg.locale = locale;
      }
    },
    destroy() {
...
var CHUNK_READ_TIMEOUT_MS = 15e3;
function readWithTimeout(reader, timeoutMs = CHUNK_READ_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    let settled = false;
...
function createTTSQueue(options) {
  ...
  const startExtendedPrefetch = () => {
    ...
    for (let i = 0; i < Math.min(2, queue.length); i++) {
      ...
      const clean = sanitizeForTTS(item.text);
      if (clean && clean.trim().length > 0) {
        const key = `${clean}\0${item.ttsModel || "default"}`;
    ...
    for (const item of toFetch) {
      const key = `${item.text}\0${item.ttsModel || "default"}`;
  ...
  const processNext = async () => {
    ...
    try {
      ...
      if (isPrefetching && nextItem) {
        ...
        setTimeout(() => {
          isProcessingNext = false;
          if (!isCancelled && gen === processGeneration) processNext();
        }, 50);
        return;
      }
      const item = getNextCleanItem();
...
var VoiceKit = class {
  ...
  constructor(config2, deps) {
    ...
    // Language detection state (for locale="auto")
    this.detectedLocale = null;
    this.isAutoLocale = false;
    // Core
  ...
  async _doInit() {
    try {
      ...
      if (this.isAutoLocale) {
        this.languageDetector = this.deps.languageDetectorProvider ?? createKondLanguageDetector({
          apiKey: this.config.apiKey,
          token: this.config.token,
          baseUrl: this.config.baseUrl,
          confidenceThreshold: this.config.languageDetection?.confidenceThreshold || 0.8,
          debug: this.config.debug,
          onLanguageChange: (lang, confidence, locale) => {
            this.handleLanguageChange(lang, confidence, locale);
          }
        });
        await this.languageDetector.init();
        if (this.config.debug) {
          console.log("[VoiceKit] Language detector initialized (auto-detect mode)");
        }
      }
      const initialLocale = this.isAutoLocale ? this.config.languageDetection?.defaultLocale || "fr" : this.locale === "multi" ? "fr" : this.locale;
  ...
  handleFinalTranscript(text, confidence) {
    ...
    if (this.isAutoLocale && this.languageDetector && text.length >= 10) {
      this.languageDetector.detect(text).catch((error) => {
        if (this.config.debug) {
          console.warn("[VoiceKit] Language detection failed:", error);
        }
      });
    }
   ...
   */
  handleLanguageChange(lang, confidence, locale) {
    this.detectedLocale = locale;
    if (this.turnManager && (locale === "fr" || locale === "en")) {
      this.turnManager.setLocale(locale);
    }
    if (this.config.debug) {
      console.log(`[VoiceKit] Language detected: ${lang} (${(confidence * 100).toFixed(0)}%) \u2192 locale: ${locale}`);
    }
    this.config.onLanguageChange?.(lang, confidence, locale);
  }
   ...
   */
  getEffectiveLocale() {
    if (this.isAutoLocale && this.detectedLocale) {
      return this.detectedLocale;
    }
    return this.locale;
  }
  ...
  destroy() {
    ...
    const safeDestroy = (name, obj) => {
      try {
        obj?.destroy();
      } catch (e) {
        if (this.config.debug) console.warn(`[VoiceKit] ${name}.destroy() failed:`, e);
      }
    };
    safeDestroy("turnDetector", this.turnDetector);
    safeDestroy("turnManager", this.turnManager);
    safeDestroy("recorder", this.recorder);
    this.recorder = null;
    safeDestroy("languageDetector", this.languageDetector);
    this.languageDetector = null;
    this.detectedLocale = null;
    this.isInitialized = false;
    if (this.config.debug) {
...
import { useState as useState4, useCallback as useCallback9, useRef as useRef9, useEffect as useEffect5 } from "react";
...
function useAudioSetup(options = {}) {
  ...
  const [isAudioActive, setIsAudioActive] = useState2(false);
  const [audioError, setAudioError] = useState2(null);
  ...
  const cleanupAudio = useCallback2(() => {
  ...
  const setupAudio = useCallback2(async () => {
    cleanupAudio();
    setAudioError(null);
    try {
...
function useSTTConnection(options = {}) {
  ...
  const reconnectWithBackoff = useCallback3(
    async (callbacks, language = "multi") => {
  ...
  const sendAudio = useCallback3((data) => {
    if (sttRef.current?.isStreaming()) {
      sttRef.current.sendAudio(data);
    }
  }, []);
  const isConnected = useCallback3(() => {
    return sttRef.current !== null;
  }, []);
  const isStreaming = useCallback3(() => {
...
function useVoiceTimeouts(options) {
  ...
  const idleTimeoutRef = useRef4(null);
  const maxSessionTimeoutRef = useRef4(
    null
  );
  const speechFinalTimeoutRef = useRef4(
    null
  );
  const triggeredTimeoutRef = useRef4(
    null
  );
  const silenceTimeoutRef = useRef4(null);
  const streamingSafetyTimeoutRef = useRef4(
    null
  );
  const vadCooldownTimeoutRef = useRef4(
    null
  );
  const maxUtteranceTimeoutRef = useRef4(
    null
  );
  const lastSpeechTimeRef = useRef4(Date.now());
  const log = useCallback4(
    (...args) => {
      if (debug) console.log("[voice-timeouts]", ...args);
    },
    [debug]
  );
  const clearIdleTimeout = useCallback4(() => {
    if (idleTimeoutRef.current) {
      clearTimeout(idleTimeoutRef.current);
      idleTimeoutRef.current = null;
    }
  }, []);
  const clearMaxSessionTimeout = useCallback4(() => {
    if (maxSessionTimeoutRef.current) {
      clearTimeout(maxSessionTimeoutRef.current);
      maxSessionTimeoutRef.current = null;
    }
  }, []);
  const clearSpeechFinalTimeout = useCallback4(() => {
    if (speechFinalTimeoutRef.current) {
      clearTimeout(speechFinalTimeoutRef.current);
      speechFinalTimeoutRef.current = null;
    }
  }, []);
  const clearTriggeredTimeout = useCallback4(() => {
    if (triggeredTimeoutRef.current) {
      clearTimeout(triggeredTimeoutRef.current);
      triggeredTimeoutRef.current = null;
    }
  }, []);
  const clearSilenceTimeout = useCallback4(() => {
    if (silenceTimeoutRef.current) {
      clearTimeout(silenceTimeoutRef.current);
      silenceTimeoutRef.current = null;
    }
  }, []);
  const clearStreamingSafetyTimeout = useCallback4(() => {
    if (streamingSafetyTimeoutRef.current) {
      clearTimeout(streamingSafetyTimeoutRef.current);
      streamingSafetyTimeoutRef.current = null;
    }
  }, []);
  const clearVADCooldownTimeout = useCallback4(() => {
    if (vadCooldownTimeoutRef.current) {
      clearTimeout(vadCooldownTimeoutRef.current);
      vadCooldownTimeoutRef.current = null;
    }
  }, []);
  const clearMaxUtteranceTimeout = useCallback4(() => {
  ...
  const startIdleTimeout = useCallback4(() => {
  ...
  const resetIdleTimeout = useCallback4(() => {
    lastSpeechTimeRef.current = Date.now();
    if (idleTimeoutMs > 0) {
      startIdleTimeout();
    }
  }, [idleTimeoutMs, startIdleTimeout]);
  const startMaxSessionTimeout = useCallback4(() => {
  ...
  const scheduleSpeechFinalCommit = useCallback4(() => {
    clearSpeechFinalTimeout();
  ...
  const cancelSpeechFinalCommit = useCallback4(() => {
    let cancelled = false;
    if (speechFinalTimeoutRef.current) {
      clearSpeechFinalTimeout();
      cancelled = true;
  ...
  const startTriggeredTimeout = useCallback4(() => {
    clearTriggeredTimeout();
  ...
  const startSilenceTimeout = useCallback4(() => {
    clearSilenceTimeout();
  ...
  const startStreamingSafetyTimeout = useCallback4(() => {
    clearStreamingSafetyTimeout();
  ...
  const startVADCooldownTimeout = useCallback4(() => {
    clearVADCooldownTimeout();
  ...
  const startMaxUtteranceTimeout = useCallback4(() => {
    clearMaxUtteranceTimeout();
...
import { useCallback as useCallback5, useRef as useRef5, useEffect as useEffect2 } from "react";
function useTurnCoordination(options) {
  ...
  const turnManagerRef = useRef5(null);
  ...
  const createTurnManagerFn = useCallback5(() => {
  ...
  const destroyTurnManager = useCallback5(() => {
    if (turnManagerRef.current) {
  ...
  const resetTurnManager = useCallback5(() => {
    if (turnManagerRef.current) {
      log("Resetting TurnManager");
      turnManagerRef.current.reset();
    }
  }, [log]);
  const feedTranscript = useCallback5(
  ...
  const feedRMS = useCallback5((level) => {
    turnManagerRef.current?.handleRMS(level);
  }, []);
  const feedVADEvent = useCallback5((event) => {
    turnManagerRef.current?.handleVADEvent(event);
  }, []);
  const feedVADProbability = useCallback5((probability) => {
    turnManagerRef.current?.handleVADProbability(probability);
  }, []);
  const syncState = useCallback5((state) => {
  ...
  const getTranscript = useCallback5(() => {
    return turnManagerRef.current?.getTranscript() ?? "";
  }, []);
  const setTurnDetector = useCallback5(
    (detector) => {
      if (turnManagerRef.current) {
...
function useLlmIntegration(options) {
  ...
  const llmBufferRef = useRef6("");
  const llmAbortRef = useRef6(null);
  const isRequestingRef = useRef6(false);
  const log = useCallback6(
    (...args) => {
      if (debug) console.log("[llm-integration]", ...args);
    },
    [debug]
  );
  const startLlmEarly = useCallback6(
    async (transcript) => {
      if (!sendMessage) {
  ...
  const clearBuffer = useCallback6(() => {
    llmBufferRef.current = "";
  }, []);
  const hasBufferedResponse = useCallback6(() => {
    return llmBufferRef.current.length > 0;
  }, []);
  const getBufferedResponse = useCallback6(() => {
...
import { useCallback as useCallback7, useRef as useRef7, useEffect as useEffect3 } from "react";
function useSileroVAD(options = {}) {
  ...
  const vadRef = useRef7(null);
  const log = useCallback7(
    (...args) => {
      if (debug) console.log("[silero-vad]", ...args);
    },
    [debug]
  );
  const startVAD = useCallback7(
  ...
  const getSpeechProbability = useCallback7(() => {
    return vadRef.current?.getSpeechProbability() ?? 0;
  }, []);
  const isVADRunning = useCallback7(() => {
...
import { useCallback as useCallback8, useRef as useRef8, useState as useState3, useEffect as useEffect4 } from "react";
...
function useTurnDetector(options = {}) {
  ...
  const [activeType, setActiveType] = useState3(null);
  const [lastPrediction, setLastPrediction] = useState3(
    null
  );
  const [error, setError] = useState3(null);
  const providerRef = useRef8(null);
  const initializingRef = useRef8(false);
  const init = useCallback8(async () => {
    console.log(
  ...
  const addTurn = useCallback8((turn) => {
  ...
  const getProvider = useCallback8(() => providerRef.current, []);
  useEffect4(() => {
    return () => {
      if (providerRef.current) {
...
function useVoiceConversation(options = {}) {
  ...
  const [userSpeaking, setUserSpeaking] = useState4(false);
  const stateRef = useRef9("idle");
  const isActiveRef = useRef9(false);
  const finalTranscriptRef = useRef9("");
  const lastInterimRef = useRef9("");
  const streamingStartedRef = useRef9(false);
  const speechStartedTimeRef = useRef9(0);
  const maxUtteranceStartedRef = useRef9(false);
  const lastUserMessageRef = useRef9("");
  const interruptionContextRef = useRef9(null);
  ...
  const commitResponseRef = useRef9(async () => {
  });
  const sttLanguage = locale === "en" ? "en" : locale === "fr" ? "fr" : "multi";
  const resumeListeningRef = useRef9(async () => {
  });
  ...
  const startLlmEarly = useCallback9(
    async (text) => {
  ...
  const handleAutoStop = useCallback9(
    (reason) => {
  ...
  const resumeListening = useCallback9(async () => {
  ...
  const handleStreamingStart = useCallback9(() => {
  ...
  const handleTTSSpeakingStart = useCallback9(() => {
  ...
  const handleStreamingEnd = useCallback9(async () => {
  ...
  const commitResponse = useCallback9(async () => {
    timeouts.clearTriggeredTimeout();
    timeouts.clearMaxUtteranceTimeout();
    maxUtteranceStartedRef.current = false;
    const text = turn.getTranscript() || finalTranscriptRef.current || lastInterimRef.current;
  ...
  const getInterruptionContext = useCallback9(() => {
    return interruptionContextRef.current;
  }, []);
  const clearInterruptionContext = useCallback9(() => {
...
function BargeInIcon({ size = "1rem", color = "#ff9500", className = "" }) {
  const sizeStyle = typeof size === "number" ? `${size}px` : size;
  return /* @__PURE__ */ jsxs(
    "svg",
    {
...
function VADMeterIcon({
  ...
  const inactive = inactiveColor || (color === "currentColor" ? "rgba(255,255,255,0.2)" : `${color}40`);
  ...
  return /* @__PURE__ */ jsx(
    "div",
    {
      ...
      children: Array.from({ length: barCount }).map((_, i) => {
        ...
        const heightPercent = 40 + i * 60 / (barCount - 1);
        return /* @__PURE__ */ jsx(
          "div",
          {
            style: {
...
var KEYFRAMES_INJECTED2 = { current: false };
function injectKeyframes2() {
  if (KEYFRAMES_INJECTED2.current || typeof document === "undefined") return;
  KEYFRAMES_INJECTED2.current = true;
  const style = document.createElement("style");
  style.textContent = `
...
function StatusDot({ displayState, color, showGlow = true }) {
  injectKeyframes2();
  const baseSize = 8;
  ...
  switch (displayState) {
    ...
    case "processing":
      if (motion2) {
        const MotionDiv = motion2.div;
        return /* @__PURE__ */ jsx3("div", { style: { display: "flex", alignItems: "center", gap: "2px" }, children: [0, 1, 2].map((i) => /* @__PURE__ */ jsx3(
          MotionDiv,
          {
        ...
        return /* @__PURE__ */ jsxs3(
          "div",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsx3("div", { style: { width: sizeStyle, height: sizeStyle, backgroundColor: color } })
            ]
          }
        );
      }
...
var VoiceStatusIndicator = memo(function VoiceStatusIndicator2({
  ...
  const content = /* @__PURE__ */ jsxs3(
    "div",
    {
      ...
      className,
      children: [
        /* @__PURE__ */ jsx3(StatusDot, { displayState, color, showGlow }),
...
// src/react/components/VoiceWaveform.tsx
import { memo as memo2, useEffect as useEffect6, useState as useState5, useRef as useRef10 } from "react";
...
var motion3 = null;
var AnimatePresence2 = null;
try {
  const fm = __require("framer-motion");
  motion3 = fm.motion;
  AnimatePresence2 = fm.AnimatePresence;
} catch {
}
var KEYFRAMES_INJECTED3 = { current: false };
function injectKeyframes3() {
  if (KEYFRAMES_INJECTED3.current || typeof document === "undefined") return;
  KEYFRAMES_INJECTED3.current = true;
  const style = document.createElement("style");
  style.textContent = `
...
var VoiceWaveform = memo2(function VoiceWaveform2({
  isActive,
  audioLevel,
  color = "#00ff88",
  barCount = 5,
  ...
  const [idleHeights, setIdleHeights] = useState5(
    Array(barCount).fill(minHeight)
  );
  const animationRef = useRef10(void 0);
  useEffect6(() => {
    if (!isActive || audioLevel !== void 0) {
  ...
  if (!isActive) return null;
  const heights = audioLevel !== void 0 ? Array(barCount).fill(0).map((_, i) => getBarHeight(i, audioLevel)) : idleHeights;
  ...
  if (motion3 && AnimatePresence2) {
    const MotionDiv = motion3.div;
    return /* @__PURE__ */ jsx4(
      MotionDiv,
      {
        ...
        exit: { opacity: 0 },
        transition: { duration: 0.15 },
        children: heights.map((height, i) => /* @__PURE__ */ jsx4(
          MotionDiv,
          {
            style: {
              width: `${barWidth}px`,
  ...
  return /* @__PURE__ */ jsx4("div", { className, style: containerStyle, children: heights.map((height, i) => /* @__PURE__ */ jsx4(
    "div",
    {
      style: {
        width: `${barWidth}px`,
        height: audioLevel !== void 0 ? `${height}%` : void 0,
        backgroundColor: color,
        borderRadius: `${barWidth / 2}px`,
        animation: audioLevel === void 0 ? `vk-waveform-bar-${i % 5} ${0.5 + i % 3 * 0.1}s ease-in-out infinite` : void 0,
        animationDelay: audioLevel === void 0 ? `${i * 0.1}s` : void 0,
        transition: audioLevel !== void 0 ? "height 0.1s ease-out" : void 0
      }
    },
    i
  )) });
...
export {
  ...
  useVoiceKit,
  use_voice_kit_default as useVoiceKitDefault,
  useVoiceTimeouts
};
