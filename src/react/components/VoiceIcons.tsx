 ...
 */
export function BargeInIcon({ size = "1rem", color = "#ff9500", className = "" }: IconProps) {
  const sizeStyle = typeof size === "number" ? `${size}px` : size;
  return (
    <svg
      className={className}
 ...
 */
export interface VADMeterIconProps extends IconProps {
  ...
  return (
    ...
    >
      {Array.from({ length: barCount }).map((_, i) => {
        ...
        // Height increases progressively for each bar
 ...
 */
export interface AnimatedVADMeterProps extends Omit<VADMeterIconProps, "level"> {
...
export function AnimatedVADMeter({
  size = 24,
  level = 0,
  barCount = 5,
  activeColor = "#00ff88",
