...
export function createTTSQueue(options: TTSQueueOptions): TTSQueue {
  ...
  // Configure audio chunk capture for recording (if provided)
  if (onAudioChunk) {
  ...
  const PREFETCH_TIMEOUT_MS = 5000; // 5 seconds max wait for N+1 prefetch

  // Mutex: prevent re-entrant calls to processNext()
  ...
  const processNext = async () => {
    // Guard: cancelled, already playing, or re-entrant call
    ...
    try {
      ...
      if (nextPreloaded && nextItem) {
        ...
        if (gen !== processGeneration) return; // stale
      ...
      // TIMEOUT FIX: Don't wait forever if prefetch is stuck/slow
      if (isPrefetching && nextItem) {
        const elapsed = Date.now() - prefetchStartTime;
        if (elapsed > PREFETCH_TIMEOUT_MS) {
          // Prefetch is taking too long - fall back to streaming
        ...
        // Keep isProcessingNext = true during the wait, release in setTimeout
        setTimeout(() => {
          isProcessingNext = false;
      ...
      if (isFinished) {
        log("Queue complete");
        // Cleanup extended prefetch cache (prevents memory leak)
  ...
  return {
    ...
    finish: () => {
      ...
      if (!isPlaying && queue.length === 0 && !nextPreloaded) {
        // Cleanup extended prefetch cache
        for (const preloaded of extendedPrefetchMap.values()) {
          cancelPrefetch(preloaded);
        }
        extendedPrefetchMap.clear();
    ...
    cancel: () => {
      log("Cancelled");
      isCancelled = true;
      processGeneration++; // Invalidate any in-flight processNext
      queue.length = 0;
