...
export function sanitizeForTTS(text: string): string {
  ...
  // 7. Remove learned: annotations (memory badges) - should be visual only
  // Format: `learned:content` → removed entirely (not read aloud)
  sanitized = sanitized.replace(/`learned:[^`]+`/g, "");

  // === MARKDOWN FORMATTING ===

  // 8. Remove bold markers: **text** → text, __text__ → text
  sanitized = sanitized.replace(/\*\*([^*]+)\*\*/g, "$1");
  sanitized = sanitized.replace(/__([^_]+)__/g, "$1");

  // 9. Remove italic markers: *text* → text, _text_ → text
  sanitized = sanitized.replace(/\*([^*]+)\*/g, "$1");
  sanitized = sanitized.replace(/_([^_]+)_/g, "$1");

  // 10. Remove heading markers (# at line start)
  sanitized = sanitized.replace(/^#{1,6}\s+/gm, "");

  // 11. Remove list markers (- or * at line start, numbered lists)
  sanitized = sanitized.replace(/^[\s]*[-*]\s+/gm, "");
  sanitized = sanitized.replace(/^[\s]*\d+\.\s+/gm, "");

  // 12. Remove blockquote markers (> at line start)
  sanitized = sanitized.replace(/^>\s*/gm, "");

  // 13. Remove horizontal rules (---, ***, ___)
  sanitized = sanitized.replace(/^[-*_]{3,}$/gm, "");

  // 14. Remove table syntax (pipes → spaces, separator rows removed)
  sanitized = sanitized.replace(/\|/g, " ");
  sanitized = sanitized.replace(/^[\s]*[-:]+[\s]*$/gm, "");

  // 15. Remove remaining short inline code backticks: `code` → code
  sanitized = sanitized.replace(/`([^`]+)`/g, "$1");

  // === CLEANUP ===

  // 16. Clean up excessive whitespace (multiple newlines → single space)
