...
function findSentenceBoundary(text: string, startFrom: number = 0): number {
  for (let i = startFrom; i < text.length; i++) {
    ...
    // Skip surrogate pairs (emoji, etc.) — not sentence boundaries
 ...
 */
/** Reduced MIN_LENGTH for the first sentence to reduce time-to-first-audio */
const FIRST_SENTENCE_MIN_LENGTH = 10;
...
export function createSentenceAccumulator(
  ...
  return {
    append: (text: string) => {
      ...
      // Adaptive MIN_LENGTH: use shorter threshold for first sentence
      const effectiveMinLength = sentenceCount === 0
        ? FIRST_SENTENCE_MIN_LENGTH
        : (options?.minLength ?? CHUNK_CONFIG.MIN_LENGTH);
