...
export function createTurnManager(
  ...
  // RMS log throttling (avoid spamming ~100 logs/sec)
  ...
  return {
    ...
    handleRMS(level: number): void {
      ...
      // Debug logging when in speaking state - THROTTLED to avoid spam
      ...
      // ═══════════════════════════════════════════════════════════════════════
      // RMS-based barge-in detection (fallback when Silero VAD is not loaded)
      // This ensures barge-in works even if Silero fails to load from CDN
      // ═══════════════════════════════════════════════════════════════════════
      if (rmsAboveThreshold && !vadSpeechActive) {
        // DISABLED for "speaking" state: TTS audio bleeds into mic causing false positives.
        // When KOND speaks, the speaker audio is picked up by the mic with RMS ~0.05,
        // but Silero VAD correctly identifies it as NOT human speech (vadSpeechActive=false).
        // Real user speech during TTS is detected by Silero VAD (handleVADEvent, line 588).
        // RMS fallback is only useful during cooldown/streaming when VAD might not be loaded.
        if (currentState === "cooldown" || currentState === "streaming") {
          ...
          cfg.onBargeIn();
          return; // Important: don't continue normal flow after barge-in
        }
      }
    ...
    handleVADEvent(event: "started" | "ended"): void {
      ...
      if (event === "started") {
        ...
        // Include "streaming" to handle race condition where TTS starts but
        // state hasn't transitioned to "speaking" yet (onStart callback delay)
    ...
    setState(state: "listening" | "triggered" | "streaming" | "speaking" | "cooldown" | "vad_cooldown"): void {
      ...
      if (prevState === state) return; // Skip if no actual change
    ...
    reset(): void {
      ...
      lastPrediction = null;
      pendingPrediction = null;
      predictionGeneration++; // Invalidate in-flight .finally()
