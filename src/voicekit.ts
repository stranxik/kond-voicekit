...
import type { LanguageDetectorPort } from "./ports/language-detector";
...
import { createKondLanguageDetector } from "./adapters/language-detector";
...
export class VoiceKit {
  ...
  private languageDetector: LanguageDetectorPort | null = null;

  // Language detection state (for locale="auto")
  private detectedLocale: Locale | null = null;
  private isAutoLocale = false;
  ...
  constructor(config: VoiceKitConfig, deps?: VoiceKitDeps) {
    ...
    this.config = {
      ...
      languageDetection: { ...DEFAULT_CONFIG.languageDetection, ...config.languageDetection },
  ...
  private async _doInit(): Promise<void> {
    try {
      ...
      // Initialize Language Detector if locale="auto"
      ...
      // For "auto" mode, we start with defaultLocale and update dynamically.
      // For "multi" mode, use "fr" patterns by default.
      const initialLocale = this.isAutoLocale
        ? (this.config.languageDetection?.defaultLocale || "fr")
        : (this.locale === "multi" ? "fr" : this.locale as "fr" | "en");
  ...
  private handleFinalTranscript(text: string, confidence: number): void {
    ...
    // Detect language if in auto mode (fire and forget - don't block transcript flow)
   ...
   */
  private handleLanguageChange(lang: string, confidence: number, locale: Locale): void {
    this.detectedLocale = locale;

    // Update turn manager locale for semantic patterns
  ...
  destroy(): void {
    ...
    // Cleanup adapters (try/catch each to ensure full cascade)
    const safeDestroy = (name: string, obj: { destroy: () => void } | null | undefined) => {
