...
export class SileroVADAdapter implements VADProvider {
  ...
  private readonly baseRetryDelay = 1000; // 1s, 2s, 3s with exponential backoff
   ...
   */
  private async loadVADWithRetry(
    ...
    try {
      ...
      // Retry if we haven't exceeded max retries and still active
      if (this.loadAttempts < this.maxRetries && this.isActive && this.stream) {
        ...
        // Check if still active after delay (user might have stopped)
  ...
  stop(): void {
    ...
    this.stream = null;
    this.loadAttempts = 0; // Reset retry counter for next start
